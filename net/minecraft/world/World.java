...
public abstract class World implements IBlockAccess {
    ...
    private final List<Entity> loadedEntityList = Lists.newArrayList();
    private final List<TileEntity> loadedTileEntityList = Lists.newArrayList();
    private final List<TileEntity> tickableTileEntities = Lists.newArrayList();
    private final List<EntityPlayer> playerEntities = Lists.newArrayList();
    ...
    public BiomeGenBase getBiomeGenForCoords(final BlockPos pos) {
        if (this.isBlockLoaded(pos)) {
            ...
            try {
                ...
                crashreportcategory
                        .addCrashSectionCallable("Location", () -> CrashReportCategory.getCoordinateInfo(pos));
    ...
    private boolean isValid(BlockPos pos) {
        return pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos
                .getZ() < 30000000 && pos.getY() >= 0 && pos.getY() < 256;
    }
    ...
    public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos) {
        ...
        return block.getMaterial().isOpaque() && block.isFullCube() || (block instanceof BlockStairs ?
                ...
                block instanceof BlockSlab ? iblockstate.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP :
                        block instanceof BlockHopper || block instanceof BlockSnow && iblockstate
                                .getValue(BlockSnow.LAYERS) == 7);
    }
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags) {
        if (!this.isValid(pos)) {
            ...
            if (iblockstate == null) {
                ...
                if (block.getLightOpacity() != block1.getLightOpacity() || block.getLightValue() != block1
                        .getLightValue()) {
    ...
    public int getLight(BlockPos pos, boolean checkNeighbors) {
        if (pos.getX() >= -30_000_000 && pos.getZ() >= -30_000_000 && pos.getX() < 30_000_000 && pos
                .getZ() < 30_000_000) {
    ...
    public BlockPos getHeight(BlockPos pos) {
        ...
        if (pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000) {
            if (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true)) {
                i = this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4)
                        .getHeightValue(pos.getX() & 15, pos.getZ() & 15);
            } else {
                i = 0;
            }
        } else {
    ...
    public MovingObjectPosition rayTraceBlocks(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid,
        ...
        if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
            if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
                ...
                if ((!ignoreBlockWithoutBoundingBox || block
                        .getCollisionBoundingBox(this, blockpos, iblockstate) != null) && block
                ...
                while (k1-- >= 0) {
                    ...
                    if (!ignoreBlockWithoutBoundingBox || block1
                            .getCollisionBoundingBox(this, blockpos, iblockstate1) != null) {
    ...
    public void playSoundToNearExcept(EntityPlayer player, String name, float volume, float pitch) {
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
            this.worldAccesses.get(i)
                    .playSoundToNearExcept(player, name, player.posX, player.posY, player.posZ, volume, pitch);
        }
    }
    ...
    public boolean addWeatherEffect(Entity entityIn) {
        this.getWeatherEffects().add(entityIn);
        return true;
    }
    ...
    public boolean spawnEntityInWorld(Entity entityIn) {
        ...
        if (!flag && !this.isChunkLoaded(i, j, true)) {
            ...
            if (entityIn instanceof EntityPlayer) {
                ...
                this.getPlayerEntities().add(entityplayer);
                this.updateAllPlayersSleepingFlag();
            }

            this.getChunkFromChunkCoords(i, j).addEntity(entityIn);
            this.getLoadedEntityList().add(entityIn);
    ...
    private void spawnParticle(int particleID, boolean p_175720_2_, double xCood, double yCoord, double zCoord,
        ...
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
            this.worldAccesses.get(i)
                    .spawnParticle(particleID, p_175720_2_, xCood, yCoord, zCoord, xOffset, yOffset, zOffset,
                            p_175720_15_);
        }
    }

    protected void onEntityAdded(Entity entityIn) {
        EventManager.call(new SpawnCheckEvent(entityIn));
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
    ...
    public void removePlayerEntityDangerously(Entity entityIn) {
        ...
        if (entityIn instanceof EntityPlayer) {
            this.getPlayerEntities().remove(entityIn);
            this.updateAllPlayersSleepingFlag();
        }
        ...
        this.getLoadedEntityList().remove(entityIn);
                ...
                if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        iblockstate1.getBlock()
                                .addCollisionBoxesToList(this, blockpos$mutableblockpos, iblockstate1, bb, list,
                                        entityIn);
                    }
                }
            }
                ...
                if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) {
                    for (int i2 = k - 1; i2 < l; ++i2) {
                        ...
                        iblockstate.getBlock()
                                .addCollisionBoxesToList(this, blockpos$mutableblockpos, iblockstate, bb, list, null);
                    }
                }
            }
        }
    ...
    public void addTileEntity(TileEntity tile) {
        boolean flag = this.getLoadedTileEntityList().add(tile);

        if (flag && tile instanceof ITickable) {
            this.getTickableTileEntities().add(tile);
        }
    }
    ...
    public void addTileEntities(Collection<TileEntity> tileEntityCollection) {
        if (this.processingLoadedTiles) {
            ...
            for (TileEntity tileentity : tileEntityCollection) {
                this.getLoadedTileEntityList().add(tileentity);

                if (tileentity instanceof ITickable) {
                    this.getTickableTileEntities().add(tileentity);
                }
            }
        }
    }
    ...
    public void updateEntities() {
        ...
        for (int i = 0; i < this.getWeatherEffects().size(); ++i) {
            Entity entity = this.getWeatherEffects().get(i);
            ...
            if (entity.isDead) {
                this.getWeatherEffects().remove(i--);
            }
        }
        ...
        for (int i1 = 0; i1 < this.getLoadedEntityList().size(); ++i1) {
            Entity entity2 = this.getLoadedEntityList().get(i1);
        ...
        Iterator<TileEntity> iterator = this.getTickableTileEntities().iterator();
        ...
        while (iterator.hasNext()) {
            ...
            if (tileentity.isInvalid()) {
                iterator.remove();
                this.getLoadedTileEntityList().remove(tileentity);
        ...
        if (!this.addedTileEntityList.isEmpty()) {
            for (int j1 = 0; j1 < this.addedTileEntityList.size(); ++j1) {
                ...
                if (!tileentity1.isInvalid()) {
                    if (!this.getLoadedTileEntityList().contains(tileentity1)) {
                        this.addTileEntity(tileentity1);
                    }

                    if (this.isBlockLoaded(tileentity1.getPos())) {
                        this.getChunkFromBlockCoords(tileentity1.getPos())
                                .addTileEntity(tileentity1.getPos(), tileentity1);
                    }
                    ...
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (block.getMaterial() == materialIn) {
                            double d0 = (float) (l1 + 1) - BlockLiquid
                                    .getLiquidHeightPercent(iblockstate.getValue(BlockLiquid.LEVEL));
    ...
    public void removeTileEntity(BlockPos pos) {
        ...
        if (tileentity != null && this.processingLoadedTiles) {
            ...
            if (tileentity != null) {
                this.addedTileEntityList.remove(tileentity);
                this.getLoadedTileEntityList().remove(tileentity);
                this.getTickableTileEntities().remove(tileentity);
            }
    ...
    protected void setActivePlayerChunksAndCheckLight() {
        ...
        for (int i = 0; i < this.getPlayerEntities().size(); ++i) {
            EntityPlayer entityplayer = this.getPlayerEntities().get(i);
        ...
        if (!this.getPlayerEntities().isEmpty()) {
            int k1 = this.rand.nextInt(this.getPlayerEntities().size());
            EntityPlayer entityplayer1 = this.getPlayerEntities().get(k1);
    ...
    protected void playMoodSoundAndCheckLight(int p_147467_1_, int p_147467_2_, Chunk chunkIn) {
        ...
        if (this.ambientTickCountdown == 0 && !this.isRemote) {
            ...
            if (block.getMaterial() == Material.air && this.getLight(blockpos) <= this.rand.nextInt(8) && this
                    .getLightFor(EnumSkyBlock.SKY, blockpos) <= 0) {
                EntityPlayer entityplayer = this
                        .getClosestPlayer((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D, 8.0D);

                if (entityplayer != null && entityplayer
                        .getDistanceSq((double) j + 0.5D, (double) l + 0.5D, (double) k + 0.5D) > 4.0D) {
        ...
        if (!(f > 0.15F)) {
            if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
                ...
                if ((block == Blocks.water || block == Blocks.flowing_water) && iblockstate.getValue(BlockLiquid.LEVEL) == 0) {
                    ...
                    boolean flag = this.isWater(pos.west()) && this.isWater(pos.east()) && this
                            .isWater(pos.north()) && this.isWater(pos.south());
        ...
        for (int i1 = i; i1 <= j; ++i1) {
            for (int j1 = k; j1 <= l; ++j1) {
                if (this.isChunkLoaded(i1, j1, true)) {
                    this.getChunkFromChunkCoords(i1, j1)
    ...
    public <T extends Entity> List<T> getPlayers(Class<? extends T> playerType, Predicate<? super T> filter) {
        ...
        for (Entity entity : this.getPlayerEntities()) {
    ...
    public int countEntities(Class<?> entityType) {
        ...
        for (Entity entity : this.getLoadedEntityList()) {
            if ((!(entity instanceof EntityLiving) || !((EntityLiving) entity).isNoDespawnRequired()) && entityType
    ...
    public void loadEntities(Collection<Entity> entityCollection) {
        this.getLoadedEntityList().addAll(entityCollection);
    ...
    public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean p_175716_3_, EnumFacing side, Entity entityIn,
        ...
        AxisAlignedBB axisalignedbb =
                p_175716_3_ ? null : blockIn.getCollisionBoundingBox(this, pos, blockIn.getDefaultState());
        return (axisalignedbb == null || this.checkNoEntityCollision(axisalignedbb, entityIn)) && (block
                .getMaterial() == Material.circuits && blockIn == Blocks.anvil || block.getMaterial()
                .isReplaceable() && blockIn.canReplace(this, pos, side, itemStackIn));
    }
    ...
    public boolean isBlockPowered(BlockPos pos) {
        return this.getRedstonePower(pos.down(), EnumFacing.DOWN) > 0 || this
                .getRedstonePower(pos.up(), EnumFacing.UP) > 0 || this
                .getRedstonePower(pos.north(), EnumFacing.NORTH) > 0 || this
                .getRedstonePower(pos.south(), EnumFacing.SOUTH) > 0 || this
                .getRedstonePower(pos.west(), EnumFacing.WEST) > 0 || this
                .getRedstonePower(pos.east(), EnumFacing.EAST) > 0;
    }
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double distance) {
        ...
        for (int i = 0; i < this.getPlayerEntities().size(); ++i) {
            EntityPlayer entityplayer1 = this.getPlayerEntities().get(i);
    ...
    public void joinEntityInSurroundings(Entity entityIn) {
        ...
        if (!this.getLoadedEntityList().contains(entityIn)) {
            this.getLoadedEntityList().add(entityIn);
        }
    }
    ...
    public float getThunderStrength(float delta) {
        return (this.prevThunderingStrength + (this.thunderingStrength - this.prevThunderingStrength) * delta) * this
                .getRainStrength(delta);
    }
    ...
    public boolean canLightningStrike(BlockPos strikePosition) {
        if (!this.isRaining()) {
            ...
            return !biomegenbase.getEnableSnow() && !this.canSnowAt(strikePosition, false) && biomegenbase
    ...
    public Random setRandomSeed(int p_72843_1_, int p_72843_2_, int p_72843_3_) {
        long i = (long) p_72843_1_ * 341873128712L + (long) p_72843_2_ * 132897987541L + this.getWorldInfo()
                .getSeed() + (long) p_72843_3_;
        this.rand.setSeed(i);
        return this.rand;
    }
    ...
    public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
        ...
        crashreportcategory.addCrashSectionCallable("All players", () -> World.this.getPlayerEntities().size() + " total; " + World.this.getPlayerEntities().toString());
    ...
    public Block getBlock(double p_147439_1_, double p_147439_2_, double p_147439_3_) {
        return this.getBlock((int) p_147439_1_, (int) p_147439_2_, (int) p_147439_3_);
    }
